...
int32_t Ymodem_Receive (uint8_t *buf)
{
  ...
  flashdestination = ApplicationAddress;	//IAP 基地址
              ...
              else
              {
                if (packets_received == 0)
                {
                  ...
                  if (packet_data[PACKET_HEADER] != 0)
                  {
                    ...
                    if (size > (FLASH_SIZE*4))	//	IAP CODE SIZE
                    {
                      /* End session */
                      Send_Byte(CA);
                      Send_Byte(CA);
                    ...
                    //FLASH_If_Erase(APPLICATION_ADDRESS);	//擦除flash
                    Send_Byte(ACK);
                    Send_Byte(CRC16);
                  }
                ...
                else
                {
                  ...
                  if (IAP_Ram_Code(&flashdestination, (uint32_t*) ramsource, (uint16_t) packet_length/4)  == 0)
                  {
                    Send_Byte(ACK);
                  }
